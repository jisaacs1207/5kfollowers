package io.github.jisaacs1207.followers;

import org.bukkit.event.Listener;

public class PlayerConfig  implements Listener{
	long infofirstjoined;
	long infolastjoined;
	int maxFollowers;
	int currentFollowers;
	int followersDead;
	int completedAdventures;
	int failedAdventures;
	int successfulAdventures;
	String follower1Name;
	int follower1Class;
	int follower1Gene;
	int follower1Gender;
	int follower1Perk1;
	int follower1Perk2;
	int follower1Level;
	int follower1Armor;
	int follower1Weapon;
	boolean follower1Insured;
	int follower1Successes;
	int follower1Failures;
	int follower1MissionType;
	int follower1MissionLevel;
	int follower1MissionTimeLeft;
	String follower2Name;
	int follower2Class;
	int follower2Gene;
	int follower2Gender;
	int follower2Perk1;
	int follower2Perk2;
	int follower2Level;
	int follower2Armor;
	int follower2Weapon;
	boolean follower2Insured;
	int follower2Successes;
	int follower2Failures;
	int follower2MissionType;
	int follower2MissionLevel;
	int follower2MissionTimeLeft;
	String follower3Name;
	int follower3Class;
	int follower3Gene;
	int follower3Gender;
	int follower3Perk1;
	int follower3Perk2;
	int follower3Level;
	int follower3Armor;
	int follower3Weapon;
	boolean follower3Insured;
	int follower3Successes;
	int follower3Failures;
	int follower3MissionType;
	int follower3MissionLevel;
	int follower3MissionTimeLeft;
	String available1Name;
	int available1Class;
	int available1Gene;
	int available1Gender;
	int available1Perk1;
	int available1Perk2;
	int available1Level;
	String available2Name;
	int available2Class;
	int available2Gene;
	int available2Gender;
	int available2Perk1;
	int available2Perk2;
	int available2Level;
	String available3Name;
	int available3Class;
	int available3Gene;
	int available3Gender;
	int available3Perk1;
	int available3Perk2;
	int available3Level;
	String available4Name;
	int available4Class;
	int available4Gene;
	int available4Gender;
	int available4Perk1;
	int available4Perk2;
	int available4Level;
}